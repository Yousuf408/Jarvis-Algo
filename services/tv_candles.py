"""
TradingView authenticated 5-minute candle backfill for Advance ORB.
Uses the chart feed, not the scanner's daily bar.
"""
...
from core.config import (
    TV_USERNAME, TV_PASSWORD, IST,
    TV_TOKEN_TTL, TV_LOGIN_COOLDOWN, TV_CANDLE_TTL
)
from core.constants import SLOT_LENGTH

logger = logging.getLogger("tv_candles")

# ================================================================
# Caching
# ================================================================
_TV_TOKEN_CACHE: tuple[float, str] | None = None
_TV_CANDLE_CACHE: dict[tuple[str, str], tuple[float, tuple | None] | None] = {}
_TV_CACHE_LOCK = threading.Lock()
_TV_LOGIN_FAIL_AT: float | None = None
_TV_FAIL_MARKER = os.path.join(
    __import__("tempfile").gettempdir(), "tv_login_fail.ts"
)
...
def _series_rows(raw: str) -> list[list[float]]:
    """Parse the series payload format from TV chart websocket."""
...
def _yesterday_high(rows: list[list[float]]) -> float | None:
    """Max 5-min bar high of the previous trading day."""
    today = dt.datetime.now(IST).date()
...
def batch_tv_opening_candles(
    ...
    """
    Fetch today's exact opening bar + yesterday's daily high from TradingView.
    
    Returns a 5-tuple per symbol:
    (open, high, low, close, yesterday_high)
    
    All values come from authenticated TradingView chart feed.
    """
    result = {str(s).strip().upper(): None for s in symbols if s}
    timeframe = int(timeframe)
    
    # Before the opening candle closes, nothing valid to fetch
    ...
    with _TV_CACHE_LOCK:
        for symbol in list(result):
            ...
            if cached and now_ts - cached[0] < TV_CANDLE_TTL:
    ...
    with _TV_CACHE_LOCK:
        token = (
            _TV_TOKEN_CACHE[1]
            if _TV_TOKEN_CACHE and now_ts - _TV_TOKEN_CACHE[0] < TV_TOKEN_TTL
            else ""
        )
        fail_at = _TV_LOGIN_FAIL_AT if _TV_LOGIN_FAIL_AT is not None else _read_fail_marker()
        login_blocked = fail_at is not None and now_ts - fail_at < TV_LOGIN_COOLDOWN
    
    if not token and login_blocked:
        logger.warning("TradingView login skipped: throttled %.0f sec ago", now_ts - (fail_at or 0))
        return result
    ...
    try:
        ...
        if not token:
            ...
            if not token:
                logger.warning("TradingView login did not return auth_token")
